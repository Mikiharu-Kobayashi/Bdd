    ...
    if st.button("選択した企業で詳細分析・レポート生成", key="exec_analysis"):
        ...
        if not final_competitors:
            ...
            with st.spinner("📡 財務データを抽出中..."):
                ...
                if error_targets:
                    st.warning(f"一部の企業データが取得できませんでした（スキップします）: {', '.join(error_targets)}")

                if not summary_results:
                    st.error("財務データの取得に失敗しました。しばらく待ってから再試行するか、別の企業を選択してください。")
                else:
                    ...
                    with st.spinner("📝 市場分析およびバリューアップ仮説を生成中..."):
                        ...
                        与えられた財務データ（{table_str}）を起点に、一般的な市場解説ではなく、「この会社は、具体的にどのレバーを引けば企業価値（EV）が2倍、3倍になるか？」という視点で、ドライかつ論理的に記述してください。
                        ...
                        3. 稼ぐ力と資本効率の定量的比較：
                           3-1.営業利益率とROEの相関から見る、競合各社の「参入障壁」と「経営効率」の差を生む構造的要因（コスト構造、アセットライトモデル等）の解剖
                               DuPont分析的視点：ROEを「売上高純利益率 × 総資産回転率 × 財務レバレッジ」の要素に分解し、競合との差を特定
                           3-2.競合各社の財務数値から透けて見える「各社の勝ちパターン/強み」の特定
                               ユニットエコノミクス推論：財務数値から、各社の顧客獲得コストやLTVの構造を推測
