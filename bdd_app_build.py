...
with st.sidebar:
    ...
    if api_key:
        ...
        try:
            ...
            selected_model = "gemini-1.5-flash" # フォールバック
...
if st.button("Deep Analysis 開始"):
    ...
    以下の最新財務比較データに基づき、「{query}」に関連する市場環境と競合他社の分析レポートを作成してください。
    ...
    【レポート構成】
    1. エグゼクティブ・サマリー：
       業界全体のファンダメンタルズ/競合優位性/ {query}の現状および強みを踏まえた成長戦略。
       1-1.事業戦略
       1-2.人事戦略
       1-3.財務戦略
    ...
    4. {query}（非上場）への戦略的提言：
       4-1.マーケットと競合、  {query}の公開データから取りうる戦略オプションの洗い出し
